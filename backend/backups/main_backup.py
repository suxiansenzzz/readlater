...
try:
    from backend.rules import (
        ...
        get_all_rules, get_rule, update_rule, delete_rule, apply_rules_to_all
    )
except ImportError:
    import sys
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
...
async def save_article(article: ArticleCreate):
    ...
    try:
        ...
        if "timeout" in error_msg.lower():
            ...
            error_type = "network"
        elif "captcha" in error_msg.lower() or "验证" in error_msg:
            error_type = "captcha"
...
async def api_retry_fetch_error(error_id: int):
    ...
    try:
        ...
        # 尝试重新抓取
        try:
            data = fetch_article(url, title if title else None)
